...
<form method="post" name="snmpConfig" action="admin/snmpConfig">
  ...
  <div class="TwoColRight">
      ...
      <div class="boxWrapper">
       ...
       <p><b>通知状态</b>:只有此设置为<em>开启</em>时，通知才被发送出去。这是一个系统级的设置。只要此设置为<em>关闭</em>系统则不会创建通知。
			通知的当前状态也可以在本系统右上角的界面看到，<em>通知 开启</em> 或 <em>通知 关闭</em>。
        </p>
        
        <p><b>配置自动发现</b>:配置IP地址(单一地址或地址段)给本系统定期执行pings，以便及时发现新的节点。
         </p>

        <p><b>交换机配置管理</b>:对交换机进行手工或定时的备份和恢复操作。
        </p>
        
	<P><B><B>通过IP配置SNMP团体名</B>:配置的团体名用来进行SNMP数据采集和其它SNMP操作。本系统附带了一个默认为"public"的团体名。
	...
	<P><B>配置接口SNMP数据采集</B>:这里你可以配置哪些接口进行SNMP数据采集。
        ...
        <p><b>管理接口和服务</b>:<em>管理</em>意味着本系统将对接口或服务执行测试。在这里可以设置是否启用测试。
     ...
     <p><b>发送事件</b>:允许你建立一个特定的事件，并将其发送到系统中。</p>
        ...
        <p><b>配置通知</b>:创建和管理通知升级计划，称为<em>目标路径</em>。目标路径和本系统事件相关联。
            每个路径可以有任意数量的上报或目标(用户，组，角色)，并且通知可以通过电子邮件，寻呼机等等发送。本系统事件可能进一步和相关的接口或服务一起触发目标路径。
        </p>
	...
	<p><b>管理设备配置导入</b>:添加节点，接口和服务到本系统或通过设备配置导入灵活的发现网络。
	</p>

        <p><b>导入和导出资产信息</b>:从本系统导入和导出资产清单。大多数电子表格和数据库都支持逗号分隔的文件格式。通过此链接可以使用导入和导出功能。
        </p>
