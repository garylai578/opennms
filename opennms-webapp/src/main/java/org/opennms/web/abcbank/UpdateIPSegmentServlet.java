...
public class UpdateIPSegmentServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try{
            ...
            BankLogWriter.getSingle().writeLog("用户[" + userId + "]修改IP段[" + startIP + "-" + endIP + "]，部门名称修改为：" + bankname + "；备注修改为：" + comment);
            request.setAttribute("update", "true");
            response.setContentType("text/html;charset=UTF-8");
            PrintWriter pw=response.getWriter();
            pw.print("<script language='javascript'>alert('修改成功！');window.location=('/opennms/abcbank/ipsegment.jsp?update=true&curPage="
